...
public class AlumnoDAO {
	...
	public static void añadir(Alumno alumno) {
		...
		String insert = "INSERT INTO alumnos(nombre, apellidos, telefono, direccion, email, numMatricula) VALUES ('" + alumno.getNombre() + "', '" +
																														alumno.getApellidos() + "', " + 
																														alumno.getTelefono() + ", '" +
																														alumno.getDireccion() + "', '" +
																														alumno.getEmail() + "', " + 
            ...
            try {
    			...
    			System.out.println("El alumno "+ alumno.getNombre() + " " + alumno.getApellidos() + " con matricula " + alumno.getNumMatricula() + " ya existe.");
	...
	// Busca un alumno en la base de datos a partir de su nombre y apellidos e imprime todos us datos por consola
	public static Alumno buscar(String nombre, String apellidos) {
		...
		try {
			...
			if(rs.next()) {
				aux = new Alumno(rs.getString("nombre"), rs.getString("apellidos"), rs.getInt("telefono"), rs.getString("direccion"), rs.getString("email"));
	...
	// Busca un alumno en la base de datos a partir de su identificador e imprime todos us datos por consola
	...
	// Modifica un alumno dado de alta en la base de datos buscandolo por un numero de matricula, 
	// y pasandole todos los nuevos valores por parámetros
	public static void modificar(Integer numMatricula, String nombreNuevo, String apellidosNuevos, Integer telefonoNuevo, String direccionNueva, String emailNuevo) {
		
		String consulta = "UPDATE alumnos SET nombre='" + nombreNuevo +
												"', apellidos='" + apellidosNuevos +
												"', telefono=" + telefonoNuevo +
												", direccion='" + direccionNueva + 
												"', email='" + emailNuevo + "' WHERE numMatricula=" + numMatricula;
		...
		try {
			...
			if(rs!=0) {
				...
				System.out.println("El alumno con número de matrícula " + numMatricula + " se ha modificado correctamente.\n");
			}else
				System.out.println("El alumno con número de matrícula " + numMatricula + " no existe.\n" );
